...
func main() {
	...
	flag.UintVar(&aaSettings.LogMaxPrefix, "log-aggregation-max-prefix", 23, "log each aggregation into network with prefix up to value; must be <=31 where 31 means disable aggregation logging at all")
	flag.UintVar(&aaSettings.IntensiveMinPrefix, "intensive-aggregation-min-prefix", 24, "minimal prefix for intensive auto aggregation; must be <=31 where 31 means disable intensive auto aggregation")
	flag.IntVar(&aaSettings.IntensiveMinNets, "intensive-aggregation-min-nets", 2, "minimal networks count for intensive auto aggregation; must be >=2")
	flag.Uint64Var(&aaSettings.IntensiveMaxFake, "intensive-aggregation-max-fake-ips", 128, "maximum number of fake IPs for intensive auto aggregation")
	flag.Uint64Var(&aaSettings.MaxFake, "aggregation-max-fake-ips", 1024, "maximum number of fake IPs for auto aggregation; 0 means disable auto aggregation (but keep intensive auto aggregation)")
	flag.Float64Var(&aaSettings.LoFakePercent, "aggregation-lo-fake-percent", 0.2, "acceptable percent (\"1\"=100%) of fake IPs when aggregating 2 networks; must be >0")
	flag.Float64Var(&aaSettings.HiFakePercent, "aggregation-hi-fake-percent", 0.51, "acceptable percent (\"1\"=100%) of fake IPs when aggregating \"aggregation-hi-fake-percent-nets\" or more networks; must be > aggregation-lo-fake-percent")
	flag.IntVar(&aaSettings.HiFakePercentNets, "aggregation-hi-fake-percent-nets", 8, "see \"aggregation-hi-fake-percent\" description; must be >2")
